...
only if the required context remains discoverable.
...
and facts a cheap unambiguous lookup already supplies. Link maintained scripts,
configs, schemas, and tests instead of translating their implementation into
prose. Preserve rationale the source cannot explain.
...
A pointer should say when to open its target and what question it answers.
Keep common requirements near the entrypoint and specialized rules near their
...
The root is a compact operating contract, not merely a contents page. Inline
...
Capture its task, prerequisites, useful decisions, proof, and recovery. Avoid
formalizing a first attempt or retaining a session transcript as instructions.
