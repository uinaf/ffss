...
func TestParseChangeRequestURL(t *testing.T) {
	g := NewGitHub(nil)
	ref, err := g.ParseChangeRequestURL("https://github.com/uinaf/slopshipper/pull/38")
	if err != nil || ref.Owner != "uinaf" || ref.Repo != "slopshipper" || ref.Number != 38 {
		t.Fatalf("ref=%+v err=%v", ref, err)
	}
	if ref.String() != "uinaf/slopshipper#38" {
		t.Fatalf("ref string: %s", ref.String())
	}
	for _, invalid := range []string{
		"",
		"https://gitlab.com/uinaf/slopshipper/pull/38",
		"https://github.com/uinaf/slopshipper/issues/38",
		"https://github.com/uinaf/slopshipper/pull/0",
		"https://github.com/uinaf/slopshipper/pull/38/files",
		"http://github.com/uinaf/slopshipper/pull/38",
		"https://github.com/uinaf/slopshipper/pull/38?diff=split#discussion",
	} {
		var forgeErr *Error
...
// SLOPSHIPPER_FORGE_LIVE_PR=https://github.com/OWNER/REPO/pull/N
func TestObserveLive(t *testing.T) {
	url := os.Getenv("SLOPSHIPPER_FORGE_LIVE_PR")
	if url == "" {
		t.Skip("set SLOPSHIPPER_FORGE_LIVE_PR to run the live observation read")
	}
	g := NewGitHub(nil)
