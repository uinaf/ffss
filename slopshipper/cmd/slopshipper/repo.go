...
func cmdRepo(st *store.Store, args []string, opts runOptions) int {
	...
	if verb == "register" && found {
		return writeFailure(opts, 2, fmt.Errorf("%w: this repo is already registered; inspect it with slopshipper repo show or change it with slopshipper repo update", machine.ErrBadArgs))
	}
	if verb == "update" && !found {
		return mapErr(fmt.Errorf("%w: this repo has no profile; create one with slopshipper repo register", machine.ErrNotFound), opts)
	}
	profile := current
	if verb == "register" {
