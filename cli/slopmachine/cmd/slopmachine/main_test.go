...
func TestAgentDocsUseCanonicalStatusMask(t *testing.T) {
	for _, path := range []string{
		...
		filepath.Join("..", "..", "..", "..", "skills", "slopmachine", "SKILL.md"),
		filepath.Join("..", "..", "..", "..", "skills", "slopmachine", "references", "status.md"),
	} {
...
func TestDryRunStoreOpeningBranches(t *testing.T) {
	...
	t.Setenv("SLOPMACHINE_DB", missing)
...
func TestUsageMentionsStorageAndServe(t *testing.T) {
	got := usage()
	for _, command := range []string{"slopmachine storage", "slopmachine serve"} {
...
func TestVerifyFailureReturnsNonzeroAndCanRetry(t *testing.T) {
	...
	if code != 6 || !strings.Contains(out, "state=BLOCKED") || !strings.Contains(out, "slopmachine retry") {
...
func TestRunEntryPointsAndFullRecoveryFlow(t *testing.T) {
	...
	t.Cleanup(func() { _ = os.Chdir(oldDir) })
	t.Setenv("SLOPMACHINE_DB", filepath.Join(t.TempDir(), "direct.sqlite"))
...
func TestCLIFirstRunCreatesDefaultStateAndReturnsBootstrapStatus(t *testing.T) {
	...
	if bootstrap.State != "UNINITIALIZED" || bootstrap.NextAction != "slopmachine init" ||
		len(bootstrap.AllowedCommands) != 1 || bootstrap.AllowedCommands[0] != "init" || bootstrap.Blocker == "" {
		t.Fatalf("bootstrap status: %+v", bootstrap)
	}
	databasePath := filepath.Join(stateRoot, "slopmachine", "slopmachine.sqlite")
...
func TestRunReviewersRegistryAndCustomFlowInProcess(t *testing.T) {
	...
	t.Setenv("SLOPMACHINE_DB", filepath.Join(t.TempDir(), "registry.sqlite"))
...
func TestCLIStoreFailureIncludesResolvedPathAndCause(t *testing.T) {
	...
	databasePath := filepath.Join(blockingFile, "slopmachine.sqlite")
	h.env = append(h.env, "SLOPMACHINE_DB="+databasePath)

	out, code := h.run("init", "--run", "blocked")
	if code != 2 || !strings.Contains(out, databasePath) || !strings.Contains(out, "not a directory") ||
		!strings.Contains(out, "SLOPMACHINE_DB") || !strings.Contains(out, "slopmachine storage --json") {
	...
	if failure.OK || failure.Error.Kind != "state_unavailable" || failure.Error.ExitCode != 2 ||
		!strings.Contains(failure.Error.Message, databasePath) || !strings.Contains(failure.Error.Message, "SLOPMACHINE_DB") {
...
func TestDirectErrorMappingJSONAndServeBindFailure(t *testing.T) {
	...
	t.Cleanup(func() { _ = os.Chdir(oldDir) })
	t.Setenv("SLOPMACHINE_DB", filepath.Join(t.TempDir(), "errors.sqlite"))
