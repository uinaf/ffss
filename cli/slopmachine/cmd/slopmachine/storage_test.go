...
func TestStorageResolutionDefaultsAndRelativeOverrides(t *testing.T) {
	...
	wantDefault, err := physicalPath(filepath.Join(xdg, "slopmachine", "slopmachine.sqlite"))
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Setenv("SLOPMACHINE_DB", filepath.Join(blockingFile, "slopmachine.sqlite"))
	out, code = captureStdoutResult(t, func() int { return run([]string{"storage", "--json"}) })
	...
	t.Setenv("SLOPMACHINE_DB", filepath.Join(".slopmachine", "slopmachine.sqlite"))
	doc, err = resolveStorage(false)
	if err != nil {
		t.Fatal(err)
	}
	wantLocal, err := physicalPath(filepath.Join(repoDir, ".slopmachine", "slopmachine.sqlite"))
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := resolveStorage(true); !errors.Is(err, errUnsafeStatePath) || !strings.Contains(err.Error(), "/.slopmachine/") {
		t.Fatalf("unsafe storage error=%v", err)
	}

	mustWrite(t, filepath.Join(repoDir, ".git", "info", "exclude"), "/.slopmachine/\n")
	doc, err = resolveStorage(true)
...
func TestPathWithinUsesFilesystemIdentity(t *testing.T) {
	...
	inside, relative, err := pathWithin(root, filepath.Join(caseVariant, ".slopmachine", "slopmachine.sqlite"))
	if err != nil || !inside || filepath.ToSlash(relative) != ".slopmachine/slopmachine.sqlite" {
...
func TestStorageRequiresSQLiteSidecarsToBeIgnored(t *testing.T) {
	...
	withWorkingDirectory(t, repoDir)
	t.Setenv("SLOPMACHINE_DB", filepath.Join(".slopmachine", "slopmachine.sqlite"))
	mustWrite(t, filepath.Join(repoDir, ".git", "info", "exclude"), "/.slopmachine/slopmachine.sqlite\n")

	_, err := resolveStorage(true)
	if !errors.Is(err, errUnsafeStatePath) || !strings.Contains(err.Error(), "slopmachine.sqlite-wal") {
...
func TestStorageRejectsTrackedDatabaseEvenWhenPatternMatches(t *testing.T) {
	...
	if err := os.Mkdir(filepath.Join(repoDir, ".slopmachine"), 0o700); err != nil {
		t.Fatal(err)
	}
	database := filepath.Join(repoDir, ".slopmachine", "slopmachine.sqlite")
	mustWrite(t, database, "tracked")
	runGit(t, repoDir, "add", ".slopmachine/slopmachine.sqlite")
	...
	_, err := resolveStorage(true)
	if !errors.Is(err, errUnsafeStatePath) || !strings.Contains(err.Error(), "slopmachine.sqlite") {
...
func TestStorageUsesPhysicalPathForRepositoryScope(t *testing.T) {
	...
	t.Setenv("SLOPMACHINE_DB", filepath.Join("external-state", "slopmachine.sqlite"))
	doc, err := resolveStorage(true)
	wantExternal, pathErr := physicalPath(filepath.Join(external, "slopmachine.sqlite"))
	if pathErr != nil {
		t.Fatal(pathErr)
	}
	...
	localDir := filepath.Join(repoDir, ".slopmachine")
	...
	t.Setenv("SLOPMACHINE_DB", filepath.Join(inbound, "slopmachine.sqlite"))
	doc, err = resolveStorage(false)
...
func TestStorageCommandAndUnsafeJSONError(t *testing.T) {
	...
	h.env = replaceEnvironment(h.env, "SLOPMACHINE_DB", filepath.Join(".slopmachine", "slopmachine.sqlite"))
	...
	if _, err := os.Stat(filepath.Join(h.repoDir, ".slopmachine")); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("unsafe init created state: %v", err)
	}

	mustWrite(t, filepath.Join(h.repoDir, ".git", "info", "exclude"), "/.slopmachine/\n")
	out = h.must("storage", "--json")
...
func TestStorageHumanOutputIncludesUnsafeRecovery(t *testing.T) {
	...
	if !strings.Contains(out, "git_ignored=false") || !strings.Contains(out, "recovery=use a path outside the worktree") || !strings.Contains(out, "/.slopmachine/") {
...
func TestStorageCommandDirectOutput(t *testing.T) {
	...
	t.Setenv("SLOPMACHINE_DB", database)
	...
	out = captureStdout(t, func() int { return run([]string{"storage"}) })
	if !strings.Contains(out, "slopmachine storage") || !strings.Contains(out, "git_ignored=n/a") {
...
func TestReadJSONRejectsInteractiveStdin(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "interactive terminal") || !strings.Contains(err.Error(), "slopmachine schema") {
