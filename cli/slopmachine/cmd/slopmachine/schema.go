...
func allCommandSchemas() []commandSchema {
	...
	reviewer := stringSchema("Registered reviewer identity. Built-ins: slopguard, bugbot; register anything else (including a human sign-off identity) with slopmachine reviewers --add.")
...
func flags(names ...string) []flagSchema {
	...
	for _, name := range names {
		...
		case "check":
			typeName, description = "boolean", "Report the target release without touching the binary; exits 8 when an update is available."
		case "release":
