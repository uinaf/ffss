...
func TestDirectRepoCommand(t *testing.T) {
	...
	t.Setenv("SLOPMACHINE_DB", filepath.Join(t.TempDir(), "direct-repo.sqlite"))
...
func TestRepoProfileLifecycle(t *testing.T) {
	...
	if code != 2 || !strings.Contains(out, "slopmachine reviewers --add") {
	...
	if code != 5 || !strings.Contains(out, "slopmachine repo register") {
...
func TestRegisteredRepoDrivesContractDefaultsAndGates(t *testing.T) {
	...
	if !strings.Contains(st.NextAction, "slopmachine verify --cmd 'true'") {
