...
- The CLI validates strict JSON and resource IDs at its boundary, then
  applies state transitions through revision and guard checks.
- `--dry-run` never persists a transition, and `verify --cmd --dry-run`
  never executes the command.
- A non-dry `verify --cmd` intentionally invokes the local shell, so callers
  must not pass untrusted command text.
- Repository-local SQLite overrides fail closed unless the database, WAL,
  and shared-memory paths are untracked and ignored by Git.
