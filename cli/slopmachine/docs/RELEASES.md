...
`slopmachine` evaluates a CLI release after every successful push to protected
`main`. Conventional Commits decide whether a version is published; merges
with no consumer-facing release type stop without a tag.

## Published artifacts

Each CLI release contains:

- macOS archives with Developer ID signed and notarized binaries for amd64
  and arm64;
...
- Apple issues tickets for standalone binaries but does not support stapling
  them, so Gatekeeper retrieves the ticket online.

## Installer trust boundary

The installer script and the release tag, archive, and `checksums.txt` it
obtains all come over HTTPS from GitHub. The checksum detects a corrupt or
mismatched archive but, sharing one transport and hosting boundary, is not
independent provenance verification. The installer never modifies shell
startup files or escalates privileges.

For independent verification, download the artifacts and run the Cosign and
GitHub attestation workflow below before installing.
...
- `--check` reports without touching the binary and exits 8 when an update is
  available.
- It shares the installer's trust boundary; use the Cosign and attestation
  workflow below for independent provenance.
...
```bash
archive=slopmachine_v0.1.2_darwin_arm64.tar.gz

cosign verify-blob \
  --bundle checksums.txt.sigstore.json \
  --certificate-identity \
    "https://github.com/uinaf/ffss/.github/workflows/release-slopmachine.yml@refs/heads/main" \
...
Pull-request jobs get read-only repository access and no release secrets.
...
`uinaf-releaser` installation token scoped to `slopmachine` and
`homebrew-tap` with Contents write permission.

- The `release` Environment stores the Developer ID certificate, certificate
  password, and notary private key as secrets, and the Apple issuer, key, and
  Team IDs as environment variables.
...
step. Never delete or move a published tag to retry a release.
