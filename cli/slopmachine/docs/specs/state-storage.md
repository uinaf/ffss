...
```text
slopmachine intake --file - --run='<run>'
slopmachine review --evidence - --run='<run>'
...
`slopmachine storage --json` is read-only and does not require the database to
exist. It returns:

```json
{
  "schema_version": 1,
  "path": "/absolute/path/slopmachine.sqlite",
  "source": "environment",
  "scope": "repository",
  "exists": false,
  "git_ignored": true
...
`/.slopmachine/` is sufficient. A local override therefore looks like:

```bash
export SLOPMACHINE_DB=.slopmachine/slopmachine.sqlite
```
...
- AC4: The same relative `SLOPMACHINE_DB` resolves to one path from the worktree
...
- AC6: An ignored `.slopmachine/` override supports normal commands, dry runs,
...
- AC8: `slopmachine storage --json` reports path resolution and Git safety
...
- Automatically choosing `.slopmachine/` when user-level storage is
  unavailable.
...
| `.slopmachine/` database | Explicit override | Useful in constrained sandboxes, but unsafe as an invisible fallback. |
