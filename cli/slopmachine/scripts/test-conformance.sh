...
# next_action must be an executable slopmachine command. Placeholders in
...
assert_conformant() {
  ...
  if [[ $action != slopmachine\ * ]]; then
    printf 'conformance: next_action is not an executable slopmachine command: %s\n' "$action" >&2
    return 1
  fi
...
if [[ ${1:-} == --self-test ]]; then
  assert_conformant "slopmachine release --revision 2 --run='demo'"
  assert_conformant "slopmachine verify --cmd '<verification command>' --run='demo'"
  # shellcheck disable=SC2016 # the $() sample is a deliberate literal
  for corrupt in \
    '' \
    'run the /slopmachine skill' \
    'spawn a subagent to continue' \
    'claude -p "continue the run"' \
    'use cursor /loop until done' \
    'slopmachine status; rm -rf .' \
    'slopmachine build && slopmachine verify' \
    'slopmachine verify --cmd "$(cat /etc/hostname)"' \
    $'slopmachine build\nslopmachine verify'; do
...
scratch=$(mktemp -d "${TMPDIR:-/tmp}/slopmachine-conformance.XXXXXX")
trap 'rm -rf "$scratch"' EXIT

# next_action names the installed binary; put the fixture build on PATH the
# way a real installation would be.
mkdir -p "$scratch/bin"
binary=$scratch/bin/slopmachine
(cd "$repository_root" && go build -o "$binary" ./cmd/slopmachine)
export PATH=$scratch/bin:$PATH

workdir=$scratch/repo
mkdir -p "$workdir"
git -C "$workdir" init --quiet
export SLOPMACHINE_DB=$scratch/conformance.sqlite
cd "$workdir"
...
for _ in $(seq 1 40); do
  ...
  command_word=$(sed -n 's/^slopmachine \([a-z]*\).*/\1/p' <<<"$action")
