...
scratch=$(mktemp -d "${TMPDIR:-/tmp}/slopmachine-installer-test.XXXXXX")
...
run_installer() {
  PATH="$fake_bin:$PATH" \
    ...
    SLOPMACHINE_INSTALL_REPOSITORY_URL="${TEST_REPOSITORY_URL:-https://fixture.invalid}" \
...
assert_no_residue() {
  if find "$case_tmp" "$case_home" \
      \( -name 'slopmachine-install.*' -o -name '.slopmachine.*' \) \
      -print -quit | grep -q .; then
    printf 'installer left temporary files behind\n' >&2
    find "$case_tmp" "$case_home" \
      \( -name 'slopmachine-install.*' -o -name '.slopmachine.*' \) >&2
    exit 1
  fi
}
...
new_case
awk '$2 != "slopmachine_v1.2.3_linux_amd64.tar.gz"' \
...
new_case
awk '{ if ($2 == "slopmachine_v1.2.3_linux_amd64.tar.gz") $1 = sprintf("%064d", 0); print }' \
  "$case_fixtures/checksums.txt" > "$case_fixtures/checksums.new"
mv "$case_fixtures/checksums.new" "$case_fixtures/checksums.txt"
...
printf '#!/bin/sh\nprintf '\''old slopmachine\\n'\''\n' > \
  "$case_home/.local/bin/slopmachine"
...
new_case
mkdir -p "$case_home/.local/bin/slopmachine"
expect_failure 'destination path is a directory' run_installer
test -d "$case_home/.local/bin/slopmachine"
assert_no_residue
...
mkdir -p "$decoy_bin"
printf '#!/bin/sh\nexit 0\n' > "$decoy_bin/slopmachine"
chmod 755 "$decoy_bin/slopmachine"
...
test -x "$shadow_destination/slopmachine"
case "$output" in
  *"Warning: PATH resolves slopmachine to $decoy_bin/slopmachine"*) ;;
  *)
