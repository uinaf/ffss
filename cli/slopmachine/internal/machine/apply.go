...
func validRequiredReviewers(required, registered []ReviewerIdentity) ([]ReviewerIdentity, error) {
	...
	for _, reviewer := range required {
		...
		if _, ok := allowed[reviewer]; !ok {
			...
			return nil, fmt.Errorf("%w: reviewer %q is not registered; register it with slopmachine reviewers --add or use a built-in (slopguard, bugbot)", ErrBadArgs, reviewer)
		}
