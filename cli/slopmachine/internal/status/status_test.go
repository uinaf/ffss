...
func TestJSONFieldsAndDryRunMetadata(t *testing.T) {
	...
	if got := doc.CompactLine(); !strings.HasPrefix(got, "slopmachine dry-run dry ") {
...
func TestFromContracts(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			next: "slopmachine intake --file -", allowed: []string{"intake", "ask"},
		...
		},
		{
			...
			next:  "slopmachine release --revision 3", allowed: []string{"intake", "ask", "release"},
		...
		},
		{
			...
			units: []machine.Unit{{ID: "u1"}},
			next:  "slopmachine build", allowed: []string{"intake", "ask", "build"},
			required: []string{}, frontier: []string{"u1"}, completed: []string{},
		},
		{
			...
			next: "slopmachine verify --cmd '<verification command>'", allowed: []string{"verify", "ask", "block"},
		...
		},
		{
			...
			next: "slopmachine review --evidence -", allowed: []string{"review", "rework", "ask", "block"},
		...
		},
		{
			...
			next: "slopmachine retry --reason '<reason>'", allowed: []string{"retry"},
		...
		},
		{
			...
			next: "slopmachine decide --answer '<answer>'", allowed: []string{"decide"},
			required: []string{}, frontier: []string{}, completed: []string{},
		},
		{
			...
			next: "slopmachine deliver --evidence -", allowed: []string{"deliver", "ask"},
...
func TestEvidenceFollowsSelectedAction(t *testing.T) {
	...
	doc := From(run, units)
	if !strings.Contains(doc.NextAction, "slopmachine build") {
...
func TestParkedRecoveryOutranksObserve(t *testing.T) {
	...
	doc := From(run, units)
	if !strings.Contains(doc.NextAction, "slopmachine retry") {
...
func TestBootstrapDirectsFirstRunToInit(t *testing.T) {
	...
	if doc.NextAction != "slopmachine init" || doc.Blocker == "" {
		t.Fatalf("bootstrap action: %+v", doc)
	}
	if got := doc.CompactLine(); got != "slopmachine state=UNINITIALIZED next=slopmachine init" {
...
func TestNextActionSelectsAndQuotesRun(t *testing.T) {
	...
	want := `slopmachine decide --answer '<answer>' --run='run with '"'"' quote'`
