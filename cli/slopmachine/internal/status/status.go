...
func nextAction(run machine.Run, units []machine.Unit, selected machine.Command, ctx Context) string {
	...
	case machine.CmdIntake:
		command = "slopmachine intake --file -"
	case machine.CmdRelease:
		command = fmt.Sprintf("slopmachine release --revision %d", run.IntakeRevision)
	case machine.CmdVerify:
		...
		if ctx.VerifyCommand != "" {
			command = "slopmachine verify --cmd " + shellQuote(ctx.VerifyCommand)
		} else {
			command = "slopmachine verify --cmd '<verification command>'"
		}
	case machine.CmdReview:
		command = "slopmachine review --evidence -"
	case machine.CmdDeliver:
		command = "slopmachine deliver --evidence -"
	case machine.CmdObserve:
		command = observeAction(units)
	case machine.CmdDecide:
		command = "slopmachine decide --answer '<answer>'"
	case machine.CmdRetry:
		command = "slopmachine retry --reason '<reason>'"
	default:
		command = "slopmachine " + string(selected)
	}
...
func observeAction(units []machine.Unit) string {
	...
	if len(delivered) == 1 {
		return "slopmachine observe --signal '<signal>' --unit=" + shellQuote(delivered[0])
	}
	return "slopmachine observe --signal '<signal>' --unit '<unit>'"
}
