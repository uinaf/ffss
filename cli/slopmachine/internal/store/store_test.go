...
func TestOpenFailureBranches(t *testing.T) {
	...
	// A present file that is not a slopmachine database fails at the
...
func TestDefaultsPreferencesRekeyAndMissingRuns(t *testing.T) {
	if got := store.DefaultPath("/data", "/home/example"); got != filepath.Join("/data", "slopmachine", "slopmachine.sqlite") {
		t.Fatalf("xdg path: %q", got)
	}
	if got := store.DefaultPath("", "/home/example"); got != filepath.Join("/home/example", ".local", "share", "slopmachine", "slopmachine.sqlite") {
...
func TestOpenCreatesPrivateStateAndReportsResolvedFailures(t *testing.T) {
	root := t.TempDir()
	databasePath := filepath.Join(root, "missing", "slopmachine", "state.sqlite")
