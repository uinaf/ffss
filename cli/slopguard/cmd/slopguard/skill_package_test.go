...
func TestSkillSlopguardCommandsUseBuiltCLIFlags(t *testing.T) {
	t.Parallel()
	...
	for _, path := range paths {
		...
		for _, command := range documentedSlopguardCommands(t, path) {
...
func documentedSlopguardCommands(t *testing.T, path string) []string {
	t.Helper()
	var commands []string
	var command string
	for _, line := range strings.Split(readFile(t, path), "\n") {
		...
		if command == "" {
			if !strings.HasPrefix(line, "slopguard ") {
				continue
			}
			command = line
		} else {
