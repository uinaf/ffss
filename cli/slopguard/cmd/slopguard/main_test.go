...
func TestTopLevelAndCommandHelp(t *testing.T) {
	...
	var stdout bytes.Buffer
	if exit := run(t.Context(), []string{"--help"}, &stdout, io.Discard, dependencies{}); exit != 0 || !strings.Contains(stdout.String(), "slopguard <review|config|doctor|schema|selfupdate|version>") || !strings.Contains(stdout.String(), "canonical review or result JSON Schema") {
