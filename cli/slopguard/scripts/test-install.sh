...
grep -F "archive=slopguard_\${release_tag}_\${os}_\${architecture}.tar.gz" \
  "$installer" >/dev/null
...
make_archive() {
  ...
  mkdir -p "$payload"
  printf '#!/bin/sh\nprintf '\''slopguard v1.2.3 (fixture-%s-%s)\\n'\''\n' \
    "$os" "$architecture" > "$payload/slopguard"
  chmod 755 "$payload/slopguard"
  printf 'fixture license\n' > "$payload/LICENSE"
  printf 'fixture readme\n' > "$payload/README.md"
  tar -czf "$fixtures/slopguard_v1.2.3_${os}_${architecture}.tar.gz" \
...
(
  ...
  if command -v sha256sum >/dev/null 2>&1; then
    sha256sum slopguard_v1.2.3_*.tar.gz > checksums.txt
  else
    shasum -a 256 slopguard_v1.2.3_*.tar.gz > checksums.txt
  fi
)
...
test "$("$destination_without_home/slopguard" --version)" = \
  'slopguard v1.2.3 (fixture-linux-amd64)'
assert_no_residue
...
test "$("$custom_destination/slopguard" --version)" = \
  'slopguard v1.2.3 (fixture-linux-arm64)'
grep -Fx \
...
test "$("$case_home/.local/bin/slopguard" --version)" = \
  'slopguard v1.2.3 (fixture-darwin-amd64)'
grep -Fx \
  'https://fixture.invalid/releases/download/slopguard%2Fv1.2.3/slopguard_v1.2.3_darwin_amd64.tar.gz' \
  "$case_log" >/dev/null
assert_no_residue
