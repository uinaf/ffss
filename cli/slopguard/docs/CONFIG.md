...
- Default reasoning effort is `medium` for Codex and Claude and `high` for
  Cursor and Grok; any explicit configuration source overrides it.
...
- Cursor selected by repository, environment, or XDG engine configuration does
  ...
  config.
- An explicit `web_access: false` from any source is authoritative and makes
  Cursor fail capability preflight.
