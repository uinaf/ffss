...
func strictCredentialFailure(effective config.Effective, provider protocol.ProviderName, environment []string) *Error {
	...
	return newFailure(
		protocol.FailureAuth,
		fmt.Sprintf("%s strict isolation requires %s; set a supported API key or use --isolation native for existing provider authentication", label, strings.Join(names, " or ")),
		environment,
		nil,
	)
}
...
func strictCredentialRecovery(effective config.Effective, provider protocol.ProviderName) string {
	...
	return fmt.Sprintf("verify %s or use --isolation native for existing provider authentication", strings.Join(names, " or "))
}
