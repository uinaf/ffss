...
func TestDoctorUsesProviderPreflightWithoutModelCalls(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			fixture: func(t *testing.T) (string, string, string, config.Effective, []string, []string) {
				...
				return fake.path, fake.arguments, fake.probes, codexConfig(false, 5*time.Second), []string{"PATH=/usr/bin:/bin", "OPENAI_API_KEY=secret"}, []string{"PATH=/usr/bin:/bin", "HOME=/native/home", "CODEX_HOME=/native/codex"}
			},
		},
		{
			...
			fixture: func(t *testing.T) (string, string, string, config.Effective, []string, []string) {
				...
				return fake.path, fake.arguments, fake.probes, claudeConfig(false, 5*time.Second), []string{"PATH=/usr/bin:/bin", "ANTHROPIC_API_KEY=secret"}, []string{"PATH=/usr/bin:/bin", "HOME=/native/home", "CLAUDE_CONFIG_DIR=/native/claude", "ANTHROPIC_AUTH_TOKEN=test-helper", "ANTHROPIC_BASE_URL=https://gateway.invalid"}
			},
		},
		{
		...
		},
		{
			...
			fixture: func(t *testing.T) (string, string, string, config.Effective, []string, []string) {
				...
				return fake.path, fake.arguments, fake.probes, grokConfig(false, 5*time.Second), []string{"PATH=/usr/bin:/bin", "XAI_API_KEY=secret"}, []string{"PATH=/usr/bin:/bin", "HOME=/native/home", "GROK_HOME=/native/grok", "XAI_BASE_URL=https://gateway.invalid"}
			},
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			delegatedExecutable := doctorProbeWrapper(t, executable, nativeEnvironment[1:], "")
			delegatedDiagnostic := Doctor(t.Context(), DoctorOptions{
				Repository: t.TempDir(), Executable: delegatedExecutable, Environment: nativeEnvironment, Config: effective,
			})
			if delegatedDiagnostic.Status != DoctorReady || delegatedDiagnostic.Authentication != AuthenticationDelegated {
				t.Fatalf("delegated diagnostic = %+v", delegatedDiagnostic)
			}
			if _, err := os.Stat(arguments); !errors.Is(err, os.ErrNotExist) {
				t.Fatalf("delegated doctor invoked model: %v", err)
			}
		})
	}
}
...
func TestDoctorReportsTimeoutAndCancellationForEveryProvider(t *testing.T) {
	providers := []struct {
		name    string
		fixture func(*testing.T, time.Duration) (string, config.Effective)
	}{
		{name: "codex", fixture: func(t *testing.T, timeout time.Duration) (string, config.Effective) {
			return newFakeCodex(t, fakeCodexOptions{}).path, codexConfig(false, timeout)
		}},
	...
	for _, test := range providers {
		t.Run(test.name, func(t *testing.T) {
			timeoutExecutable, timeoutConfig := test.fixture(t, 25*time.Millisecond)
			...
			cancelExecutable, cancelConfig := test.fixture(t, 5*time.Second)
...
func TestDoctorClassifiesMissingIncompatibleTimeoutAndCancellation(t *testing.T) {
	base := codexConfig(false, 5*time.Second)
	providers := []struct {
		name      string
	...
	timeoutConfig := codexConfig(false, 25*time.Millisecond)
...
func TestDoctorUsesSingleTimeoutBudget(t *testing.T) {
	...
	diagnostic := Doctor(t.Context(), DoctorOptions{
		...
		Config: codexConfig(false, 100*time.Millisecond),
	})
...
func TestDoctorAllowsShortCredentialPlaceholderInNumericVersion(t *testing.T) {
	...
	diagnostic := Doctor(t.Context(), DoctorOptions{
		...
		Config:      codexConfig(false, 5*time.Second),
	})
