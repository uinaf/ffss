...
func Load(ctx context.Context, options Options) (Effective, error) {
	...
	if err := applyFile(&effective, filepath.Join(root, ".slopguard.yaml"), SourceRepository, false, "", root); err != nil {
		return Effective{}, err
	}
...
func resolveXDGPath(lookup func(string) (string, bool), homeDir func() (string, error)) (string, string, bool, error) {
	if value, ok := lookup("XDG_CONFIG_HOME"); ok && value != "" {
		...
		return filepath.Join(value, "slopguard", "config.yaml"), "", false, nil
	}
	home, err := homeDir()
	if err != nil {
	...
	return filepath.Join(home, ".config", "slopguard", "config.yaml"), home, true, nil
}
...
func applyEnvironment(effective *Effective, lookup func(string) (string, bool)) error {
	raw := rawConfig{}
	if value, ok := lookup("SLOPGUARD_ENGINE"); ok {
		raw.Engine = yamlString{value: value, set: true}
	}
	if value, ok := lookup("SLOPGUARD_MODEL"); ok {
		raw.Model = yamlString{value: value, set: true}
	}
	if value, ok := lookup("SLOPGUARD_REASONING_EFFORT"); ok {
		raw.ReasoningEffort = yamlString{value: value, set: true}
	}
	if value, ok := lookup("SLOPGUARD_TIMEOUT"); ok {
		raw.Timeout = yamlString{value: value, set: true}
	}
	if value, ok := lookup("SLOPGUARD_RETRIES"); ok {
		parsed, err := strconv.Atoi(value)
		if err != nil {
			return fmt.Errorf("environment SLOPGUARD_RETRIES: %w", err)
		}
		raw.Retries = yamlInt{value: int64(parsed), set: true}
	}
	if value, ok := lookup("SLOPGUARD_MAX_BYTES"); ok {
		parsed, err := strconv.ParseInt(value, 10, 64)
		if err != nil {
			return fmt.Errorf("environment SLOPGUARD_MAX_BYTES: %w", err)
		}
	...
	if value, ok := lookup("SLOPGUARD_WEB_ACCESS"); ok {
		...
		if err != nil {
			return fmt.Errorf("environment SLOPGUARD_WEB_ACCESS: %w", err)
		}
