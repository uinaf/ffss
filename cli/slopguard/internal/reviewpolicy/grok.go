...
Return the provider schema's review and completion objects. Set completion.status to complete only after assessing every target file. Include exactly one completion.files entry per target file, explain the concrete assessment for that file, and link each review finding by its zero-based index to the matching file. The completion object is internal protocol evidence and is not part of slopguard's public result schema.
