...
func (s *Store) migrate() error {
	...
	updated_at TEXT NOT NULL
		...
		case 8:
			...
			// whenever the name exists at all, and let the operator rename
			// or remove the custom identity first. Forge-reviewer VALUES are
			// logins, not identities, so only its keys are inspected.
			var occupied int
			if err := tx.QueryRow(`SELECT
				(SELECT COUNT(*) FROM reviewers WHERE name = 'slopguard')
				+ (SELECT COUNT(*) FROM runs
					WHERE required_reviewers_json LIKE '%"slopguard"%'
					   OR completed_reviewers_json LIKE '%"slopguard"%')
				+ (SELECT COUNT(*) FROM repos
					WHERE bindings_json LIKE '%"slopguard"%'
					   OR forge_reviewers_json LIKE '%"slopguard":%')`).Scan(&occupied); err != nil {
