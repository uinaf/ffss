...
func observeUnit(ctx context.Context, st *store.Store, adapter forge.Forge, repoKey, runID string, unit machine.Unit, deliveries map[string]store.Delivery, pass int, opts runOptions) (observationResult, int) {
	...
	if !ok {
		...
		result.Note = "no delivery evidence names this unit; record the signal manually with slopshipper observe"
		return result, 0
	}
	...
	if evidence.PRURL == "" {
		if evidence.CommitSHA != "" {
			...
			// incomplete answer.
			result.Note = "direct-trunk delivery is not forge-observable; record signals manually with slopshipper observe"
			return result, 0
		}
		result.ErrorKind = "unobservable"
		result.Note = "delivery recorded no change request URL; record the signal manually with slopshipper observe"
		return result, 0
	}
...
func writeWatch(doc watchDocument, opts runOptions) int {
	...
	if doc.ErrorKind != "" {
		fmt.Fprintf(os.Stderr, "slopshipper watch: %s: see slopshipper watch --help for recovery\n", doc.ErrorKind)
	}
	return 0
}
